...
const BookingSchema = new Schema(
  {
    ...
    // The fleet currently holding the exclusive acceptance window.
    assignedFleet: {
      type: Schema.Types.ObjectId,
      ref: "Fleet",
      default: null,
    },
    // When the exclusive window expires. After this, the booking escalates to the next fleet.
    assignedFleetExpiry: {
      type: Date,
      default: null,
    },
    // Ordered list of fleet IDs to try, built once at booking creation by the scoring algorithm.
    fleetAssignmentQueue: [
      {
        type: Schema.Types.ObjectId,
        ref: "Fleet",
      },
    ],
    // Audit log of every fleet that was assigned, timed out, declined, or accepted.
    fleetAssignmentHistory: [
      {
        ...
        reason: {
          type: String,
          enum: ["timeout", "declined", "accepted"],
        },
      },
    ],
  },
...
BookingSchema.index({ assignedFleet: 1, assignedFleetExpiry: 1 });
