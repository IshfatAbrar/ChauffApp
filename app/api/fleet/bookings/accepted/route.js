...
/** GET /api/fleet/bookings/accepted?driverId=<optional>
 *  Returns all accepted bookings for this fleet's drivers, sorted by pickup time asc. */
export async function GET(req) {
  ...
  if (driverIdParam) {
    ...
    catch { /* ignore bad driverId, return all */ }
  }
  ...
  const bookings = await Booking.find({ chauffeur: { $in: driverIds }, status: "accepted" })
